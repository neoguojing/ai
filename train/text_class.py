...
def evaluate(dataloader,model_path="neo_albert_model",mode="inference"):
    import csv
            ...
            for batch in dataloader:
                ...
                for i in range(len(predicted_labels)):
                    ...
                    if mode == "test":
                        ...
                        # update_predict_result(cursor,label_rev_map[label],confidence,ids[i])
                        writer.writerow([label_rev_map[label],confidence,ids[i]])
                        # 提交更改
                        print(f"user:{ids[i]},{label_rev_map[label]},{confidence}")
                counter+=len(input_ids)    
                print("current pos:",counter)
                if mode == "test":
                    # Compute accuracy
                    total += labels.size(0)
...
def do_inference():
    ...
    evaluate(test_loader,model_path="neo_albert_model92.5",mode="inference")

def update_predict_result(cursor,lable,confidence,chat_id):
    try:
        cursor.execute("UPDATE telegram_user_summary SET label = ?, confidence = ? WHERE chat_id = ?", (lable,confidence, chat_id))
        conn.commit()    
